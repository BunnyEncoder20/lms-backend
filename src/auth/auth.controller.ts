...
Which Auth to Pick:
  - Multi-application intranet that needs SSO → run Keycloak (OIDC) internally and validate tokens in Nest.
  - Single application, mostly browser users → Server-side sessions (Redis) + CSRF protection.
  - API-first / microservices → JWT bearer with short-lived access tokens + refresh token rotation and central auth service.
